...
def recommend_courses_for_semester(plan_config: PlanConfig, plan: Plan, semester: Semester, catalog: Catalog, dependency_graph: DependencyGraph) -> Dict[str, List[Union[Course, List[Course]]]]:
                ...
                for req in cat.requirements:
                    ...
                    for possible_course in possible:
                        if isinstance(possible_course, list):
                            continue
                        if possible_course.course_code and possible_course.course_code not in all_taken_codes:
        ...
        for cat in program.categories:
            for req in cat.requirements:
                ...
                for possible_course in possible:
                    if isinstance(possible_course, list):
                        continue
                    if hasattr(possible_course, 'course_code') and possible_course.course_code and possible_course.course_code not in all_taken_codes:
    ...
    for category in list(eligible_by_category.keys()):
        ...
        for group in unique_groups:
            # Eligibility check for group or single
            if len(group) > 1:
                # All must be eligible to take together (all prereqs satisfied, coreqs satisfied by group)
                group_eligible = True
                for c in group:
                        ...
                        for req in path:
                            if isinstance(req, list):
                                ...
                                if req != c and req not in all_taken_codes and req not in group:
                                    missing.append(req)
                        # Only allow missing if all are coreqs in the group
                ...
                c = group[0]
                # Use is_eligible for single course
                if is_eligible(c):
...
if __name__ == "__main__":
    ...
    # User input: list of lists of course codes for each semester
    # (Fall 2025, Spring 2026, Fall 2026, ..., Spring 2029)
    user_semester_courses = [
        # Fall 2025
        ["CHEM 1601", "CHEM 1601L", "MATH 1300", "ES 1401", "ES 1402", "ES 1403"],
        # Spring 2026
        ["PHYS 1601", "PHYS 1601L", "MATH 1301", "CS 1101"],
        # Fall 2026
    ...
    # Build semesters and collect completed codes
    ...
    for i, codes in enumerate(user_semester_courses):
        ...
        # If all semesters have courses, recommend for the next one (if within range)
    ...
    for category, courses in eligible_by_category.items():
        if not any(excluded in category for excluded in ["Open Electives", "Liberal Arts Core"]):
            print(f"\n{category}:")
